...
-- Based on ESPN NFL Teams API structure with Roster Composition Management
...
-- Roster positions table - defines the required starting lineup structure
CREATE TABLE roster_positions (
    id SERIAL PRIMARY KEY,
    position_code VARCHAR(10) NOT NULL,            -- QB, RB, WR, TE, FLEX, K, DST, BENCH
    position_name VARCHAR(50) NOT NULL,             -- "Quarterback", "Running Back", etc.
    max_count INTEGER NOT NULL,                     -- Maximum players allowed in this slot
    is_starting BOOLEAN DEFAULT true,               -- true for starting positions, false for bench
    display_order INTEGER NOT NULL,                -- Order to display positions
    
    UNIQUE(position_code)
);

-- Players table for NFL players (updated to include D/ST as a special case)
...
DECLARE
    ...
    team_total INTEGER;
BEGIN
    -- Get the maximum allowed for this position
    SELECT max_count INTO max_allowed 
    FROM roster_positions 
    WHERE position_code = NEW.roster_position;
    
    -- Count current players in this position for this team
    SELECT COUNT(*) INTO position_count
    FROM team_players 
    WHERE team_id = NEW.team_id 
    AND roster_position = NEW.roster_position
    AND (TG_OP = 'INSERT' OR id != NEW.id);
    
    -- Check if adding this player would exceed the limit
    IF position_count >= max_allowed THEN
        RAISE EXCEPTION 'Team already has maximum % players in % position', 
            max_allowed, NEW.roster_position;
    END IF;
    
    -- Check total roster size (16 players max: 9 starters + 7 bench)
    SELECT COUNT(*) INTO team_total
    FROM team_players 
    WHERE team_id = NEW.team_id
    AND (TG_OP = 'INSERT' OR id != NEW.id);
    
    IF team_total >= 16 THEN
        RAISE EXCEPTION 'Team roster is full (16 players maximum)';
    END IF;
    
    -- Validate FLEX position can only contain RB, WR, or TE
    IF NEW.roster_position = 'FLEX' THEN
        IF NOT EXISTS (
            SELECT 1 FROM players 
            WHERE id = NEW.player_id 
            AND position IN ('RB', 'WR', 'TE')
        ) THEN
            RAISE EXCEPTION 'FLEX position can only contain RB, WR, or TE players';
        END IF;
    END IF;
    
    -- Validate position compatibility (except FLEX and BENCH)
    IF NEW.roster_position NOT IN ('FLEX', 'BENCH') THEN
        IF NOT EXISTS (
            SELECT 1 FROM players 
            WHERE id = NEW.player_id 
            AND position = NEW.roster_position
        ) THEN
            RAISE EXCEPTION 'Player position % does not match roster position %', 
                (SELECT position FROM players WHERE id = NEW.player_id), 
                NEW.roster_position;
        END IF;
    END IF;
...
-- Create trigger to enforce roster composition
CREATE TRIGGER trg_validate_roster_composition
